...
import { getProblemHistory as apiGetProblemHistory } from '@/lib/api';
...
export function useProblemHistory() {
  ...
  const fetchProblems = useCallback(async () => {
    ...
    try {
      ...
      // Get problems from AWS API Gateway
      const data = await apiGetProblemHistory(user.id);
      ...
      if (data && data.length > 0) {
        ...
        const formattedProblems: ProblemEntry[] = data.map(problem => {
          ...
          const processingTimeMs = problem.processingTimeMs || problem.processing_time_ms || 0;
          ...
          return {
            ...
            type: problem.inputType || problem.input_type as 'text' | 'image' | 'voice',
            content: problem.textContent || problem.text_content || problem.imageUrl || problem.image_url || problem.voiceUrl || problem.voice_url || '',
            ...
            solvedAt: problem.solvedAt || problem.created_at || new Date().toISOString(),
            timeSpent: timeSpentMinutes,
            tags: tags,
            imageUrl: problem.imageUrl || problem.image_url || undefined,
            voiceUrl: problem.voiceUrl || problem.voice_url || undefined,
          };
        });
